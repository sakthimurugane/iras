...
$delcredqry = "UPDATE SUPPLIERS set IS_DELETED=1, modified_on=now(), modified_by='$modified_by' where supplier_id=".$profileid;
    
if ($link->query($delcredqry) === FALSE) {
   
    echo "Error: " . $delcredqry . "<br>" . $link->error;
        $errflag=true;
        $errmsg="Error while deleting suppliers. ".$link->error;
    }
    ...
    $link->close();
        ...
        if($errflag){
            $_SESSION['ERRMSG_DELSUPPLIER']=$errmsg;
        }else{
            $_SESSION['SCSMSG_DELSUPPPLIER']="Supplier has been removed from the system";
        }
