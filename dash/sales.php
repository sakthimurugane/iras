...
if(!isset($_POST['invoiceid'])){
    ...
    if(trim($_POST['invoiceid'])==''){
        ...
        if(isset($_POST['delProdSubmit']) && isset($_POST['salesid'])){
            ...
            $delsalesqry="UPDATE SALES_ORDER SET is_deleted=1, modified_by='$modified_by', modified_on=now() where status='UNBILLED' and ordernum='$invoiceid' and sale_id=$salesid";
            //echo $delsalesqry;
    ...
    <div class="container-fluid">
      ...
      <form method="post" name="addProdForm" id="addProdForm">
        <div class="card-header">
			<select name="product" id="product" style="width:400px;" class="chosen-select" data-placeholder="Select the Product....">
            <option></option>
            	<?php
            	...
            	?>
            		<option value="<?php echo $row['product_id'];?>"><?php echo $row['product_name'].' '.$row['product_code']; ?> - <?php echo $row['hsnsac']; ?>  | <?php echo '&#8377; '.formatMoney($row['s_price'],true); ?></option>
            	<?php
            				}
            			?>
            </select>
            <input type="hidden" name="invoiceid" id="invoiceid" value="<?php echo $invoiceid;?>" />
            <input type="number" style="width:60px;" name="qty" id="qty" value="1" min="1" placeholder="Qty" autocomplete="off" required>
            <input type="number" style="width:60px;" name="price" id="price" min="1" placeholder="0.00" autocomplete="off" required>
	...
	<tbody>
		<?php
			...
			<?php
				...
				if($taxflag){
				    $taxslabqry="select * from tax_slab where status='ACTIVE' and is_deleted=0";
				    $hresult=mysqli_query($link,$taxslabqry);
				    if($hresult) {
				        if(mysqli_num_rows($hresult) > 0) {
				            while($taxrow=mysqli_fetch_assoc($hresult)){
				                
				                $taxname=$taxrow['slab_name'];
				                $taxvalue=$taxrow['tax_value'];
				                $taxtype=$taxrow['tax_type'];
				                $taxamount=0;
				                $taxdisp='';
				                if($taxtype=='Fixed'){
				                    $taxamount = $taxvalue;
				                    ...
				                    $taxamount = $totalamt*($taxvalue/100);
				                    $taxdisp = $taxvalue.'%';
			...
			<td colspan="3">
			<?php echo '&#8377; '.formatMoney($taxamount,true);?>
			</td>
		...
		<tr>
			...
			echo '&#8377; '.formatMoney($totalamt, true); 
                        ...
                        <div class="modal fade" id="checkout" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
                            <div class="modal-dialog" style="width:30%;height:30%;">
                                <div class="modal-content">
                                	<form id="checkoutForm" name="checkoutForm" method="post" action="../controllers/checkoutprocessor.php">
                                    <div class="modal-header">
                                        <h4 class="modal-title text-success" id="myModalLabel"><i class="fa fa-money"></i> <span class="text-right"> Pay <?php echo '&#8377; '.formatMoney($totalamt,true)?></span></h4>
                                                ...
                                                <table class="table">
                                                    <tbody>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <input id="example-ajax-post" style="width: 268px; height:30px;" type="text" size="25" value="" name="cnd"  autocomplete="off" placeholder="Enter Customer ID" required/>
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td class="text-success">
                                                            ...
                                                            <td>  
                                                                 <div class="checkbox">
                                                                 <label>
                                                                 <input type="checkbox" name="excludetax" value="">
                                                                 <small>Without Tax <?php echo '&#8377; '.formatMoney($totalamt-$totaltax,true);?></small>
                                                                 </label>                                    
                                    ...
                                    <div class="modal-footer">
                                    	<input type="hidden" name="ordernum" value="<?php echo $invoiceid;?>" />  
                                    	<input type="hidden" name="billamount" value="<?php echo $totalamt;?>" /> 
                                    	<input type="hidden" name="taxedamt" value="<?php echo $totaltax;?>" /> 
                                    	<input type="hidden" name="profitamt" value="<?php echo $totalprofit;?>" />
                                    	                        	
                                        <button class="btn btn-success form-control" type="submit" name="checkoutSubmit"><i class="fa fa-credit-card"></i> Submit</button>
